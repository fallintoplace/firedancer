      ...
      case FD_SSHTTP_ADVANCE_DATA: {
        ...
        if( FD_UNLIKELY( !ctx->sent_meta ) ) {
          ...
          if( ctx->is_redirect ) {
            ...
            int   is_zstd = 0;
            if( FD_UNLIKELY( fd_ssarchive_parse_filename( resolved_name, &full_slot, &incremental_slot, parsed_hash, &is_zstd ) || !is_zstd ) ) {
              FD_LOG_WARNING(( "failed to parse resolved snapshot filename `%s`", resolved_name ));
              transition_malformed( ctx, stem );
              fd_sshttp_cancel( ctx->sshttp );
              break;
            }
            int is_full_filename = !strncmp( resolved_name, "snapshot-", 9UL );
