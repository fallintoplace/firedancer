...
runtime_err_abs( int err ) {
  if( err>=0 ) return (uint)err;
  long n = -(long)err;
  if( n>(long)UINT_MAX ) n = (long)UINT_MAX;
  return (uint)n;
}
...
report_runtime_txn( fd_execrp_tile_t const * ctx ) {
  ...
  /* Errors */
  ev.txn_err       = runtime_err_abs( txn_out->err.txn_err       );
  ev.exec_err      = runtime_err_abs( txn_out->err.exec_err      );
  ev.exec_err_kind = runtime_err_abs( txn_out->err.exec_err_kind );
